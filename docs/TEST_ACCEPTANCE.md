...
## 2. M2 验收（MALF Lifespan + v1.5 行为快照）⏳

### 待补单测 `tests/test_malf_lifespan.py`
...
### 待补单测 `tests/test_malf_behavior.py`
...
### 待补单测 `tests/test_pas_core.py`（核心：全枚举确定性）
...
## 4. M4 验收（Signal + 回测引擎）⏳ 最关键

### 待补单测 `tests/test_backtest_rules.py`（手算对账）

| 测试点 | 标准 | 源 |
|---|---|---|
| 初始止损 | stop = T0.low − 0.02 | 规则 3 |
| 风险单位 | 1R = entry − stop；target1 = entry + 1R | 规则 4 |
| 买入日破止损 | T1 收盘 < stop → T2 开盘清仓 | 规则 5 |
| target1 减半 | 触及 target1 → 平 50% | 规则 6 |
| 移动止损高于 target1 | 减半后 trailing 用 max(trail, target1+ε) | 规则 7 |
| 时间止损 | time_stop_bars 内无推进 → 退出 | 规则 8 |
| T+1 | 买入当日不可卖（除 T2 破线清仓规则） | A 股约束 |
| 涨停拒买 | open≥涨停价 → 买入 fill 失败 | 涨跌停 |
| 跌停拒卖 | open≤跌停价 → 卖出 fill 失败顺延 | 涨跌停 |
| R-multiple | realized_pnl / (1R × original_qty) 计算正确 | 度量 |

### L3 手算对账（必做）

```mermaid
flowchart LR
    PICK[选 1 标的<br/>2-3 笔交易] --> HAND[人工算<br/>entry/stop/target1/R]
    HAND --> RUN[程序跑出 bt_trade]
    RUN --> CMP{逐字段比对}
    CMP -->|一致| PASS[通过]
    CMP -->|不一致| FIX[修引擎]
```
...
| 检查 | 标准 |
|---|---|
| 扫描数据 | T0 扫描只用 bar_dt ≤ T0 的数据 |
| 进场时点 | 进场永远在发现日的**下一**交易日 open，绝不当日 |
| pivot 延迟 | pivot confirm_bar_dt 延迟 k 根，回测中不提前可见 |

---

## 5. M5 验收（调参 + 分组回测 + 完整 UI）⏳

| 测试点 | 标准 |
|---|---|
| 三组隔离 | initial/validation/holdout 年份不重叠（2018-20/21-23/24-26） |
| holdout 锁 | holdout 只能跑一次（运行计数锁生效） |
| 参数网格 | 笛卡尔积每点一 backtest_run，结果可复现 |
| 指标排序 | bt_metrics（expectancy/profit_factor/max_dd）排序正确 |
| 端到端 | initial→validation→holdout 全流程跑通，UI Page2 三组并排 |
...
python scripts/ingest_data.py --limit 5        # M1 灌数冒烟
streamlit run src/asteria/ui/app.py            # M1+ 肉眼核对结构
python scripts/run_backtest.py                 # M4 跑回测（手算对账）
python scripts/run_tuning.py                   # M5 三组工作流
```
